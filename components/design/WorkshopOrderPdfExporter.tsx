...
import { WorkshopOrderReport } from "@/components/design/WorkshopOrderReport";
import {
...
export type WorkshopOrderPdfExporterHandle = {
...
/**
 * يجهّز PDF أمر تشغيل الورشة ويعرض فتح/مشاركة.
 */
export const WorkshopOrderPdfExporter = forwardRef<
  WorkshopOrderPdfExporterHandle,
  Props
>(function WorkshopOrderPdfExporter(
  ...
  const openShare = useCallback(async () => {
    ...
    try {
      ...
      if (!sheet) throw new Error("تعذر تجهيز أمر التشغيل");
      if (sheet.querySelectorAll(".report-page").length === 0) {
        throw new Error("تعذر تجهيز صفحات أمر التشغيل");
      }
      const pdf = await buildNamedPdfFile(
        sheet,
        workshopOrderPdfFileName(projectName)
      );
      setFile(pdf);
      setPhase("ready");
    } catch (err) {
      console.error(err);
      setError("تعذر تجهيز ملف أمر التشغيل. حاول مرة أخرى.");
      setPhase("error");
      setExportMounted(false);
    }
  }, [phase, projectName]);
  ...
  async function handleShareClick() {
    ...
    try {
      const result: SharePdfResult = await sharePdfFile(
        file,
        `أمر تشغيل${projectName ? ` — ${projectName}` : ""}`
      );
      ...
      {showOverlay && typeof document !== "undefined"
        ? createPortal(
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="wo-share-title"
              ...
              >
                {phase === "preparing" ? (
                  <div className="py-4 text-center">
                    ...
                    >
                      جاري تجهيز أمر التشغيل…
                    </p>
                    <p className="mt-1 text-xs text-muted">
                      الرسمة والقطاع واللون والزجاج
                    </p>
                  </div>
                ) : null}
