...
import { PurchaseOrderReport } from "@/components/design/PurchaseOrderReport";
import {
...
export type PurchaseOrderPdfExporterHandle = {
...
/**
 * يجهّز PDF طلبية المشتريات ويعرض فتح/مشاركة.
 */
export const PurchaseOrderPdfExporter = forwardRef<
  PurchaseOrderPdfExporterHandle,
  Props
>(function PurchaseOrderPdfExporter(
  ...
  const openShare = useCallback(async () => {
    ...
    try {
      ...
      if (!sheet) throw new Error("تعذر تجهيز طلبية المشتريات");
      if (sheet.querySelectorAll(".report-page").length === 0) {
        throw new Error("تعذر تجهيز صفحات الطلبية");
      }
      const pdf = await buildNamedPdfFile(
        sheet,
        purchaseOrderPdfFileName(projectName)
      );
      setFile(pdf);
      setPhase("ready");
    } catch (err) {
      console.error(err);
      setError("تعذر تجهيز ملف طلبية المشتريات. حاول مرة أخرى.");
      setPhase("error");
      setExportMounted(false);
    }
  }, [phase, projectName]);
  ...
  async function handleShareClick() {
    ...
    try {
      const result: SharePdfResult = await sharePdfFile(
        file,
        `طلبية مشتريات${projectName ? ` — ${projectName}` : ""}`
      );
      ...
      {showOverlay && typeof document !== "undefined"
        ? createPortal(
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="po-share-title"
              ...
              >
                {phase === "preparing" ? (
                  <div className="py-4 text-center">
                    ...
                    >
                      جاري تجهيز طلبية المشتريات…
                    </p>
                    <p className="mt-1 text-xs text-muted">
                      تجميع القطاعات والزجاج والاكسسوار
                    </p>
                  </div>
                ) : null}
