    ...
    <div className="flex flex-col gap-4">
      <div className="flex gap-2">
        ...
        <Link
          href={
            filterProject
              ? ROUTES.accounting.expenseForProject(
                  filterProject.customerId,
                  filterProject.id
                )
              : ROUTES.accounting.newExpense
          }
      ...
      {filterProject ? (
        <div className="flex items-center justify-between gap-2 rounded-2xl border border-border bg-card px-3.5 py-2.5">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card px-4 py-10 text-center text-sm text-muted">
          لا توجد مصروفات — اضغط «مصروف» للتسجيل
        </div>
      ) : (
